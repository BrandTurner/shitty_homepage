  ...
  var twitterFetcher = {
    ...
    callback: function(data) {
      ...
      while(n < x) {
        ...
        //arraytweets.push(op);
        arrayTweets.push(tweets[n].firstChild.textContent);
        n++;
      }
      //handleTweets(arrayTweets);
      handleBeatsTweets(arrayTweets);
      inProgress = false;
...
var config1 = {
  "id": '632428622993747972',
  "domId": 'example1',
  "maxTweets": 1,
  "enableLinks": true,
  "customCallback": handleBeatsTweets,
  "showPermalinks": false,
  "printTime":false,
  "printUser":false
};
var x = twitterFetcher.fetch(config1);


function handleBeatsTweets(tweets) {
  var element_top = document.getElementById("beats_top");
  var element_bottom = document.getElementById("beats_bottom");
	var x = tweets.length;
	var songInfo = [];
	var json_data = {};
	var n = 0;
	...
	while(n < x) {
		...
		songInfo = tweets[n].split("-");
		json_data = {
			Title: songInfo[0],
			Artist: songInfo[1]
		}
    element_top.innerHTML = songInfo[0];
    element_bottom.innerHTML = songInfo[1];
